 ...
 * n → n/2 (n is even) <br>
 * n → 3n + 1 (n is odd) <br>
 * Using the rule above and starting with 13, we generate the following sequence: 13 → 40 → 20 → 10 → 5 → 16 → 8 → 4 → 2 → 1 It can be seen that this
 * sequence (starting at 13 and finishing at 1) contains 10 terms. Although it has not been proved yet (Collatz Problem), it is thought that all
 * starting numbers finish at 1. Which starting number, under one million, produces the longest chain?
...
public class Problem_14_LongestCollatzSequence
{
     ...
     * @param number
     *            The number for which collatz length has to be calculated (must be at least 1).
     * @param numberByCollatzLength
     *            The Map of known collatz length keyed by the number that has that length (cannot be null).
     ...
     * @param num
     *            The number for which collatz function has to be applied (must be at least 1).
     * @return A positive integer.
     */
